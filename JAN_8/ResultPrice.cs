				...
				foreach (FieldMetadata field in moduleData.Fields.Where(f => f.IsPriceSensitive))
				{
					...
					if (priceMap.TryGetValue(field.VpcId, out price))
					{
						if (isResidentialPricingMode)
						{
                            ...
                            //BMCA Calculate BuyPrice using ListPrice x Multiplier
                            ...
                            //percentDollarValue = actual BuyPrice returned from Interface...no need to do rounding, interface does this...
                            //BMCA calculate Rounded BuyPrice
							//netPrice = PricePCG.Effective_Price(percentDollarValue, buyPriceInfo.PriceRounding);

                            netPrice = buyPriceInfo.AdjustedListPrice;
						}
            ...
            SMARTListPriceMap = priceMap.ToDictionary(p => p.Key, p =>
                {
                    ...
                    respListReq = getSMARTPricing(listPrice,true, productPricing, result, productData, modelNumber);
