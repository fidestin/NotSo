	...
	public class ResultPricing : IPriceGenerator
	{
		...
		public ResultPriceInfo GetResultPriceInfo(Result result, ModuleMetadata moduleData, PricingState state)
		{
			...
			else
			{
				...
				{
					buyPriceMap = GetBuyPriceInfoMap(state.CostCenter, modelNumber, priceMap, productData, productPricing, result, output,orderNumberMap,moduleData);
				}
						...
						if (isResidentialPricingMode)
						{
							...
							else // accessory field
							{
								try
								{
                                    if (useSMART)
                                    {
                                        //Pass in the Accessory (e.g. 'BAYCURB042A') as the MODEL_NUMBER
                                        string accessoryModelNumber = orderNumber;
                                        SMARTResponseData respListReq = new SMARTResponseData();
                                        string listPrice = price.ToString();
                                        respListReq = getSMARTPricing(listPrice, true, productPricing, result, productData, accessoryModelNumber);
                                       

                                        buyPriceMultiplier = float.Parse(respListReq.priceMultiplier);
                                        float adjustedListPrice = float.Parse(respListReq.adjustedListPrice);
        ...
        public SMARTResponseData getSMARTPricing(string listPrice, bool accessoryPricing,ProductPricing productPricing, Result result, ProductDataJSON productData, string modelNumber)
        {
            try
            {
                ...
                //This needs to be passed in....let SMART do the pricing...
                //string listPrice = "434"; //This is a nominal number, we are just interested in getting the BuyPriceMultiplier.
		...
		public Dictionary<long, BuyPriceInfo> GetBuyPriceInfoMap(string costCenter, string modelNumber,
            Dictionary<long, float> priceMap, ProductDataJSON productData, ProductPricing productPricing, Result result, ResultPriceInfo priceInfo, Dictionary<long, string> orderNumberMap, ModuleMetadata moduleData)
		{
            ...
            SMARTListPriceMap = priceMap.ToDictionary(p => p.Key, p =>
                {
                    ...
                    if (IsModelNumberField(moduleData, p.Key))
                    {
                        isAccessory = false;
                    }
                    else
            ...
            if (productPricing.OrderNumberGenerationType == OrderNumberGenType.Split)
            {
                ...
                ordNumberToBuyPriceInfoMap = orderNumberMap.ToDictionary(p => p.Key, p =>
                {
                    ...
                    string listPrice = "100";

                    //Now price this string - Treat Split model as Accessory
                    respReq = getSMARTPricing(listPrice,true,productPricing, result, productData, splitModelName);
                    Debug.WriteLine("Split - Model :" + splitModelName + "-" + p.Key.ToString() + "---Mult: " + respReq.priceMultiplier + " : " + respReq.adjustedListPrice);
                    ...
                    return new BuyPriceInfo()
                   {
                       ...
                       PriceRounding = 2,
                       AdjustedListPrice=float.Parse(respReq.adjustedListPrice)         //BMCA added for SMART
                   };
                });
            ...
            else
            {
                typeToBuyPriceInfoMap = productPricing.GetMultGroupCodeMap(result).ToDictionary(p => p.Key, p =>
			    {
                    ...
                    {
                        string listPrice = "100";
                        respReq = getSMARTPricing(listPrice,false,productPricing, result, productData, modelNumber);

                        Debug.WriteLine("Model - Model :" + modelNumber + "-" + p.Key.ToString() + "---Mult: " + respReq.priceMultiplier);
                   
                        buyPriceMultiplier = float.Parse(respReq.priceMultiplier);
                        adjustedListPrice = float.Parse(respReq.adjustedListPrice);
			...
			else
			{
                ...
                SMARTBuyInfo = priceMap.ToDictionary(op => op.Key, op => buyPriceInfo);
