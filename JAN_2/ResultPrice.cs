...
namespace TOPSS.Pricing
{
	...
	public class ResultPricing : IPriceGenerator
	{
        ...
        public SMARTResponseData getSMARTPricing(ProductPricing productPricing, Result result, ProductDataJSON productData, string modelNumber)
        {
            try
            {
                ...
                string r12result = priceProxyWrapper.callPricing(false, sourceHeaderRef, listPrice, soldToSite, customerNumber, modelNumber, productCode, r12ProductFamilyID, ref respReq);
            ...
            Dictionary<long, float> priceMap, ProductDataJSON productData, ProductPricing productPricing, Result result, ResultPriceInfo priceInfo, Dictionary<long, string> orderNumberMap)
		{
            ...
            if (productPricing.OrderNumberGenerationType == OrderNumberGenType.Split)
            {
                ordNumberToBuyPriceInfoMap = orderNumberMap.ToDictionary(p => p.Key, p =>
                {
                    ...
                    Debug.WriteLine("Split - Model :" + splitModelName + "-" + p.Key.ToString());

                    //Now price this string?

                    //supposed to call SMART pricing here (duplicate code) to get the BuyPriceMultiplier....
                    return new BuyPriceInfo()
                   {
                       BuyPriceMultiplier = 0.36f,
